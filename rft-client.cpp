...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        while ((!allSent) && (!allAcked)) {
            ...
            datagramS ackDatagram{}; // zero-initialize to avoid stale fields if no data read
            ...
            if (recvBytes > 0) {
                ...
                if (!validateChecksum(ackDatagram)) {
                    WARNING << "Received corrupted ACK (bad checksum). Ignoring." << ENDL;
                } else {
                    TRACE << "Received acknowledgment for seqNum " << ackDatagram.ackNum << ENDL;
                    if ((ackDatagram.ackNum >= base) && (ackDatagram.ackNum < nextSeqNum)) {
                        ...
                        // ACK acknowledges something beyond what we've sent — warn and ignore
